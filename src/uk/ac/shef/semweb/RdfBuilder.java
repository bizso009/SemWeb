...
public abstract class RdfBuilder
{
    ...
    protected final String dbpediaWikiPage="http://dbpedia.org/ontology/genre";
    protected final String dppediaHomeTown="http://dbpedia.org/ontology/genre";
...
/*
    public ResultSet queryDBpedia(String res, String dbpediaProp ){
        ...
        for (int i = 0; i < 1; i++){
            selection += "?arg"+i;
            where += "<" + res + "> <" + dbpediaProps[i] + "> ?arg" + i + " . ";
        }
        String query = "SELECT " + selection + " WHERE { " + where + " }";
